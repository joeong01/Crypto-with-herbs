            ...
            <div class="about_content">
                ...
                <CModal size="md" alignment="center" :visible="add" @close="add = false;" >
                    ...
                    <CModalBody>
                        ...
                        <label>Wallet ID:&emsp;  </label><input type="text" id="walletID" placeholder="walletID" @change="isEmpty()"/><br><br>
                ...
                <CAccordion style="width: 700px;">
                    <CAccordionItem v-for="merchant in merchants" :key="merchant.merchantCategory">
                        <CAccordionHeader><b>ID: {{ merchant.merchantCategory }} &emsp; {{ merchant.merchantName }}</b>&emsp; &emsp;  fund to release: {{ merchant.fund }}&emsp; <CButton @click="releaseFund(merchant.walletID, merchant.fund)" color="primary" shape="rounded-pill">ReleaseFund</CButton></CAccordionHeader>
                        <CAccordionBody>
                            ...
                            <Label>Platform Charge:&emsp; </Label><input type="number" step="0.01" min="0.01" :value="merchant.platformCharges" :id="merchant.merchantCategory + 'charge'" @change="checking(merchant.merchantCategory)"/>
                            <div style="margin-left: 62%;">
                                ...
                                <CButton @click="remove(merchant.merchantCategory)" color="danger" shape="rounded-pill">Delete</CButton>
                            </div>
                        </CAccordionBody>
                    </CAccordionItem>
                </CAccordion>
    ...
    methods:{
        ...
        async remove(ID){
            if(confirm("Are you sure wan to delete \"" + document.getElementById(ID+'name').value + "\"?")){
                console.log(await axios.delete("http://localhost:5000/merchant/delete",{ id: ID,}));
                alert("Removed");
            }
        },
        isEmpty(){
